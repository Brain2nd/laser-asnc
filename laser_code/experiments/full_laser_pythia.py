...
def capture_calibration(model, input_ids):
    ...
    for i, layer in enumerate(layers):
        # pre-GeLU: output of dense_h_to_4h
        def _pg(idx):
            def h(m, a, kw, out):
                pre_gelu[idx].append(out.detach().float().flatten().cpu())
            return h
        hooks.append(layer.mlp.dense_h_to_4h.register_forward_hook(_pg(i), with_kwargs=True))

        # LN INPUTS (for ASNCLayerNorm which quantizes input)
        def _ln1(idx):
            def h(m, args, kwargs):
                x = args[0] if args else kwargs.get("hidden_states")
                ln1_in[idx].append(x.detach().float().flatten().cpu())
            return h
        def _ln2(idx):
            def h(m, args, kwargs):
                x = args[0] if args else kwargs.get("hidden_states")
                ln2_in[idx].append(x.detach().float().flatten().cpu())
            return h
    ...
    def softmax_spy(*args, **kwargs):
        ...
        if aid is not None:
            # Capture post-softmax output (attention weights in [0, 1])
            pre_softmax[aid].append(out.detach().float().flatten().cpu())
        return out
